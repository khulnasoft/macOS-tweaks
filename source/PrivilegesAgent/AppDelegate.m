...
{
    os_log(OS_LOG_DEFAULT, "KHULNASOFTLtd: Launched for user %{public}@", NSUserName());
...
- (BOOL)listener:(NSXPCListener *)listener shouldAcceptNewConnection:(NSXPCConnection *)newConnection
{
    ...
    // one of the components using a bundle identifier starting with "com.khulnasoft.privileges"
    NSError *error = nil;
    ...
    if (signingAuth) {
        ...
        NSString *reqString = [MTCodeSigning codeSigningRequirementsWithCommonName:signingAuth
                                                                  bundleIdentifier:@"com.khulnasoft.privileges*" 
...
#pragma clang diagnostic ignored "-Warc-retain-cycles"
                [newConnection setInvalidationHandler:^{
                    ...
                    os_log(OS_LOG_DEFAULT, "KHULNASOFTLtd: %{public}@ invalidated", newConnection);
                }];
#pragma clang diagnostic pop
                
                [newConnection resume];

                os_log(OS_LOG_DEFAULT, "KHULNASOFTLtd: %{public}@ established", newConnection);
    
            } else {
                    os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTLtd: Code signature verification failed");
            }
        ...
        os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTLtd: Failed to get code signature: %{public}@", error);
    }
            ...
            if (minutesLeft > 0) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self revokeAdminRightsWithCompletionHandler:^(BOOL success) {
                        ...
                        os_log(OS_LOG_DEFAULT, "KHULNASOFTLtd: Administrator privileges for user %{public}@ have expired.", [[self->_privilegesApp currentUser] userName]);
                    }];
                });
            }
        }];
    ...
    if (executablePath) {
        ...
        if (executableURL) {
            ...
            if ([executableURL getResourceValue:&isBundle forKey:NSURLIsPackageKey error:nil] && [isBundle boolValue]) {
                ...
                if (error) {
                    os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "%{public}@", [NSString stringWithFormat:@"KHULNASOFTLtd: Failed to launch %@: %@", [executableURL path], error]);
                }
            }
            
        } else {
            
            os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "%{public}@", @"KHULNASOFTLtd: Failed to launch executable: Invalid file url");
        }
    }
}
    ...
    if (remoteLoggingConfiguration) {
        ...
        if ([[logServerType lowercaseString] isEqualToString:kMTRemoteLoggingServerTypeSyslog]) {
            ...
            if ([[self->_privilegesApp currentUser] hasAdminPrivileges]) {
                ...
                logMessage = [NSString stringWithFormat:@"KHULNASOFTLtd: User %@ now has administrator privileges", [[self->_privilegesApp currentUser] userName]];
                if ([reason length] > 0) { logMessage = [logMessage stringByAppendingFormat:@" for the following reason: \"%@\"", reason]; }
            
            } else {
            
                logMessage = [NSString stringWithFormat:@"KHULNASOFTLtd: User %@ now has standard user privileges", [[self->_privilegesApp currentUser] userName]];
            ...
            [self writeToSyslog:logMessage user:[_privilegesApp currentUser] completionHandler:^(NSError *error) {
                ...
                if (error) {
                    os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTLtd: Remote logging failed: %{public}@", error);
                }
            }];
    ...
    if (!isRestricted || adminEnforced) {
        ...
        [_daemonConnection connectToDaemonAndExecuteCommandBlock:^{
            ...
            [[[self->_daemonConnection connection] remoteObjectProxyWithErrorHandler:^(NSError *error) {
                ...
                os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_FAULT, "KHULNASOFTLtd: Failed to connect to daemon: %{public}@", error);
                ...
                if (success) {
                    ...
                    if (!isRestricted && !adminEnforced) {
                        ...
                        if (removeAfterMinutes > 0) {
                            ...
                            os_log(OS_LOG_DEFAULT, "KHULNASOFTLtd: Administrator privileges are automatically revoked in %{public}@", [durationFormatter stringFromMeasurement:durationMeasurement]);
